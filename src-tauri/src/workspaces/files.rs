...
pub(crate) fn read_workspace_file_inner(
    ...
    let canonical_root = root
        .canonicalize()
        .map_err(|err| format!("无法解析工作区根目录：{err}"))?;
    ...
    let file = File::open(&canonical_path).map_err(|err| format!("无法打开文件：{err}"))?;
    ...
    let content =
        String::from_utf8(buffer).map_err(|_| "文件不是有效的 UTF-8 文本。".to_string())?;
