...
export function GitDiffPanel({
  ...
  const showFileMenu = useCallback(
    async (
      ...
      if (onRevertFile) {
        items.push(
          await MenuItem.new({
            text: countSuffix ? `丢弃改动${countSuffix}` : "丢弃改动",
            action: async () => {
  ...
  const perFileDiffStatusLabel = `${perFileDiffGroups.length} 个文件 · ${perFileEditCount} 处改动`;
