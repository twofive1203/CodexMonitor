...
export function SettingsShortcutsSection({
  ...
  return (
    <SettingsSection
      title="快捷键"
      subtitle="自定义文件操作、编辑器、面板和导航的快捷键。"
    >
      <div className="settings-field settings-shortcuts-search">
        ...
        <div className="settings-help">可按分组名称、操作名称或默认快捷键筛选。</div>
      </div>
      ...
      {filteredGroups.length === 0 && (
        <div className="settings-empty">
          未找到匹配的快捷键：{normalizedSearchQuery ? `"${searchQuery.trim()}"` : "当前搜索"}
        </div>
      )}
    </SettingsSection>
  );
